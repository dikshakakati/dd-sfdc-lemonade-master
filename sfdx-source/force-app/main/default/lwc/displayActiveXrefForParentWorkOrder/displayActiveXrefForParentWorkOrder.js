...
const LWC_NAME = "DisplayActiveXrefForParentWorkOrder";
...
export default class DisplayActiveXrefForParentWorkOrder extends NavigationMixin(
  LightningElement
) {
  @api recordId;
  @api viewAll;
  ...
  setXrefRecords({ error, data }) {
    if (data) {
      ...
      allData.forEach((obj) => {
        if (
          this.labels.integrationSuccess.includes(obj.Integration_Status__c)
        ) {
          successData.push(obj);
        }
      });
      this.relatedData = [...successData];
      this.showComponent = true;
    } else if (error) {
      createLog({
        lwcName: LWC_NAME,
